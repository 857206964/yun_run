...
def create_retry_session(retries=5, backoff_factor=2,
    ...
    """创建带重试机制的Session对象，增加429处理"""
    session = requests.Session()
    retry = Retry(
        total=retries,
        status=retries,  # 增加对4xx状态码的重试
        backoff_factor=backoff_factor,
        status_forcelist=status_forcelist,
        respect_retry_after_header=True,  # 遵守服务器返回的Retry-After
        allowed_methods=frozenset(['GET', 'POST'])  # 明确允许重试的方法
    )
...
def generate_random_ua():
    """生成随机移动端User-Agent"""
    ios_versions = ["14_7_1", "15_0", "15_1", "15_2", "15_3"]
    webkit_versions = ["605.1.15", "604.1", "604.3"]
    safari_versions = ["14.1.2", "15.0", "15.1", "15.2"]

    return (
        f"Mozilla/5.0 (iPhone; CPU iPhone OS {random.choice(ios_versions)} "
        f"like Mac OS X) AppleWebKit/{random.choice(webkit_versions)} "
...
def loginGetCode(user, password):
    """1、登录获取Code（带重试和超时）"""
    is_phone = "@" not in user
    url_user = user if not is_phone else f"+86{user}"

    url = f"https://api-user.huami.com/registrations/{url_user}/tokens"
    data = {
        "client_id": "HuaMi",
        "password": password,
    ...
    try:
        ...
        response = session.post(url, data=data, headers=headers, timeout=(3.05, 10), allow_redirects=False)
        response.raise_for_status()

        location = response.headers.get("Location", "")
        if not location:
            raise ValueError("Location header missing in response")

        print(f"成功获取Location: {location[:60]}...")  # 截取部分内容避免日志过长
        code = re.search("(?<=access=).*?(?=&)", location)
        return is_phone, code.group(0) if code else None

    except (requests.exceptions.RequestException, ValueError) as e:
        print(f"登录获取Code失败: {str(e)}")
        return False, None

def getLoginToken(code, is_phone):
    """2、获取login_token（增强版错误处理）"""
    ...
    try:
        session = create_retry_session()
        response = session.post(url, data=base_data, timeout=10)
        response.raise_for_status()

        result = response.json()
        print("登录响应数据:", json.dumps(result, indent=2, ensure_ascii=False))

        if not result.get("token_info"):
            print("登录失败：响应中缺少token_info字段")
            return None, None
        ...
        print(f"获取login_token失败: {str(e)}")
        return None, None

def getAppToken(login_token):
    """3、获取app_token（完整错误处理）"""
    if not login_token:
        print("无效的login_token")
        return None
    ...
    try:
        ...
        data = response.json()
        print("AppToken响应数据:", json.dumps(data, indent=2, ensure_ascii=False))

        if not data.get("token_info", {}).get("app_token"):
            print("响应中缺少有效的app_token")
            return None

        return data["token_info"]["app_token"]

    except (requests.exceptions.RequestException, json.JSONDecodeError, KeyError) as e:
        print(f"获取app_token失败: {str(e)}")
        return None

def brushStep(app_token, user_id, step):
    """4、刷步数（带数据校验）"""
    if not all([app_token, user_id, step]):
        print("缺少必要参数: app_token/user_id/step")
    ...
    try:
        # 数据预处理
        data_json = re.sub(r"date%22%3A%22.*?%22", f"date%22%3A%22{today}%22", base_data)
        data_json = re.sub(r"ttl%5C%22%3A.*?%2C", f"ttl%5C%22%3A{step}%2C", data_json)

        # 使用当前时间戳
        current_timestamp = int(time.time())
        data_json = re.sub(r"last_sync_data_time=\d+",
                           f"last_sync_data_time={current_timestamp}",
                           data_json)

        # 准备请求
        url = f'https://api-mifit-cn.huami.com/v1/data/band_data.json?&t={get_time_stamp()}'
        headers = {"apptoken": app_token, "Content-Type": "application/x-www-form-urlencoded"}
        payload = f'userid={user_id}&device_type=0&last_deviceid=DA932FFFFE8816E7&data_json={data_json}'
        ...
        result = response.json()
        print("刷步响应:", json.dumps(result, indent=2, ensure_ascii=False))
        return result.get("code") == 1, result.get("message", "Unknown response")

    except Exception as e:
        print(f"刷步操作失败: {str(e)}")
        return False, str(e)
...
def server_send(msg, sckey=None):
    """增强版Server酱推送"""
    try:
        sckey = sckey or os.getenv('SCKEY')
        if not sckey:
            print("未配置SCKEY，跳过推送")
            return
        ...
        response = create_retry_session().post(
            ...
            data={'text': '刷步通知', 'desp': msg},
            timeout=10
        )
        response.raise_for_status()
        print("推送成功，返回:", response.text[:100])
    except Exception as e:
        print(f"推送失败: {str(e)}")

def format_result(message, success):
    """统一格式化结果"""
    time_bj = datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8)))
    status = "成功" if success else "失败"
    return f"[{time_bj.strftime('%Y-%m-%d %H:%M:%S')}] 刷步{status} | {message}"

# ---------------------- 主流程函数 ----------------------
def main(user, password, step_min=30000, step_max=40000):
    """主控制流程"""
    print("\n" + "="*40 + " 开始执行 " + "="*40)

    # 参数校验
    if not all([user, password]):
        return format_result("用户名或密码不能为空", False)
    ...
    try:
        ...
        return format_result(f"系统异常: {str(e)}", False)
    finally:
        print("="*40 + " 执行结束 " + "="*40 + "\n")
...
if __name__ == '__main__':
    ...
    # 执行主程序
    result = main(user, password)
    print(result)

    # 推送结果
    server_send(result, sckey)
